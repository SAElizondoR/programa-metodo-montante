...
VentanaPrincipal::VentanaPrincipal()
    : VentanaPrincipal(new Gtk::ApplicationWindow::BaseObjectType(),
                       Glib::RefPtr<Gtk::Builder>()) {}

VentanaPrincipal::VentanaPrincipal(
    Gtk::ApplicationWindow::BaseObjectType *objeto_c,
    const Glib::RefPtr<Gtk::Builder> &constructor)
    : Gtk::ApplicationWindow(objeto_c), constructor(constructor) {
  ...
  ajuste_cant_ecuaciones->signal_value_changed().connect(
      sigc::mem_fun(*this, &VentanaPrincipal::cambiar_valor_ajuste));
  // utiliza el método de obtener_matriz de la clase VentanaPrincipal para leer
  // desde la interfaz
  // se llena la matriz con los números que se leen
  btn_resolver->signal_clicked().connect(
      sigc::mem_fun(*this, &VentanaPrincipal::obtener_matriz));
}
...
void VentanaPrincipal::cambiar_valor_ajuste() {
  ...
  lista_cuadros->remove(
      *lista_cuadros->get_children().back()); // borrar la tabla anterior
  tabla = new Gtk::Grid();
  int cant_ecuaciones =
      int(ajuste_cant_ecuaciones
              ->get_value()); // obtener nuevo número de ecuaciones
  // insertar 2 * n + 1 columnas (n + 1 para las entradas, n para los
  // separadores)
  ...
  for (int indice_ren = 0; indice_ren < cant_ecuaciones; ++indice_ren) {
    ...
    for (int indice_col = 0; indice_col < 2 * cant_ecuaciones - 1;
         ++indice_col) {
...
void VentanaPrincipal::imprimir_btn_aumentar(int indice_ren, int indice_col) {
  ...
  Glib::RefPtr<Gtk::SpinButton> btn =
      Glib::RefPtr<Gtk::SpinButton>::cast_dynamic(
          constructorCelda->get_object("NumeroCelda"));
  btn->unparent(); // elimina referencia a padre
  tabla->attach(*btn.operator->(), indice_col,
                indice_ren); // se inserta en la tabla
}

void VentanaPrincipal::imprimir_etiqueta(const std::string &sim, int indice_ren,
                                         int indice_col) {
  auto etiqueta = Gtk::make_managed<Gtk::Label>();
  // separador
...
void VentanaPrincipal::hacer_resolver_matriz() {
  ...
  if (respuesta.empty())
    ...
    for (int i = 0; i < cant_ecuaciones; ++i) {
      // cadena de respuesta
      respuesta_cadena += "x_" + std::to_string(i + 1) + ": " +
                          std::to_string(respuesta.at(i)) + ", ";
    }
...
void VentanaPrincipal::obtener_matriz() {
  ...
  for (int indice_ren = 0; indice_ren < cant_ecuaciones; ++indice_ren) {
    ...
    for (int indice_col = 0; indice_col < 2 * cant_ecuaciones + 1;
      ...
      auto btn = dynamic_cast<Gtk::SpinButton *>(
          tabla->get_child_at(indice_col, indice_ren));
